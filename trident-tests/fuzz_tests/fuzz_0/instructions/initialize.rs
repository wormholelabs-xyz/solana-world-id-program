...
use crate::fuzz_instructions::{FuzzAccounts, Initialize};
...
impl<'info> IxOps<'info> for Initialize {
    ...
    fn get_data(
        ...
        //     root_expiry: self.data.args.root_expiry,
        //     allowed_update_staleness: self.data.args.allowed_update_staleness,
        // };

        // let data = solana_world_id_program::instruction::Initialize { args: args.into() };

        let data = solana_world_id_program::instruction::Initialize {
            args: self.data.args.into(),
        };
        Ok(data)
    }
    fn get_accounts(
