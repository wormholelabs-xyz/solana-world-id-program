...
impl<'info> IxOps<'info> for PostSignatures {
    ...
    fn get_data(
        ...
        let mut signature = crate::constants::quardian_set_5_mock::SIGNATURES;

        signature[0][RANDOM_INDEX1] = self.data.guardian_signatures[0];

        let data = solana_world_id_program::instruction::PostSignatures {
            guardian_signatures: signature.to_vec(),
            total_signatures: signature.len() as u8,
        };
