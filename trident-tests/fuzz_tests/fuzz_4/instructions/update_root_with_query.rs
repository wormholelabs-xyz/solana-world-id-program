...
use crate::constants::quardian_set_5_mock::RANDOM_INDEX1;

use crate::constants::quardian_set_5_mock::REFERENCE_SIG_VALUE1;
...
impl<'info> IxOps<'info> for UpdateRootWithQuery {
    ...
    fn check(
        ...
        // this ensures that the Root Account was Initialized
        let _root_post = match post_ix.root {
            Some(r) => r,
            None => return Ok(()),
        };
        let signatures_pre = match pre_ix.guardian_signatures {
            Some(s) => s,
            None => return Ok(()),
        };

        if signatures_pre.guardian_signatures[0][RANDOM_INDEX1] != REFERENCE_SIG_VALUE1 {
            return Err(FuzzingError::Custom(0));
        }
        Ok(())
    }
}
